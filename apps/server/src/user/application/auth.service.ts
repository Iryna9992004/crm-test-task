...
export class AuthService {
  ...
  async validateGithubKey(githubKey: string): Promise<boolean> {
    try {
      const response = await axios.get(`${config.github.api}/user`, {
  ...
  async validateGithubUsername(
    githubKey: string,
    username: string,
  ): Promise<boolean> {
    try {
      const response = await axios.get(
        `${config.github.api}/users/${username}`,
        {
          headers: { Authorization: `token ${githubKey}` },
        },
      );
  ...
  async register(
    ...
    const isValidUsername = await this.validateGithubUsername(
      githubKey,
      username,
    );
    if (!isValidUsername) {
